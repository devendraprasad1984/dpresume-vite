...
const isPresent = value => isObjectPresent(value) && value !== "undefined"
  && value !== "null" && stringify(value).trim() !== "";
const isBlank = (value) => !isPresent(value);
const isAbsPresent = value => isPresent(value) && value !== "0" && value !== 0
  && value !== "0.00";
const isArrayPresent = value => Array.isArray(value) && value.length > 0;
const isDecimalValue = value => value.toString().indexOf(".") !== -1;
const areStringsEqual = (value1, value2) => nullSafeString(value1)
  === nullSafeString(value2);
const areStringsEqualIG = (value1, value2) => nullSafeString(
  value1).toLowerCase() === nullSafeString(value2).toLowerCase();
const nullSafeString = value => (isPresent(value) ? value : "");
const nullSafeInteger = value => (isNumber(value) ? value : 0);
const nullSafeTrim = (value) => (isNumber(value)) ? value : nullSafeString(
  value).trim();
const convertToString = value => (isObjectPresent(value) ? value.toString()
  : "");
const ifTrue = val => (val !== null && (val === true || val === "true" || val
  === "TRUE" || val === 1 || val === "1"));
const ifFalse = val => (val === null || val === false || val === "false" || val
  === "FALSE" || val === 0 || val === "0");
const deepCopy = obj => stringifyAndParse(obj);
const zeroIfNull = val => isAbsPresent(val) ? val : 0;
const getFileExtension = value => nullSafeString(value).substring(
  nullSafeString(value).lastIndexOf("."));
...
const getUniqueObjectsByKey = (array, key) => {
  return [...new Map(array.map(item => [item[key],
    item])).values()];
};
...
const getInputKeyVal = (e, values) => {
  ...
  const value = !isObjectPresent(values) || !isObjectPresent(values?.value)
    ? nullSafeString(e.target.value) : nullSafeString(values.value) || "";
  return {
    name,
    value
  };
...
const getCurrencySymbol = (locale, currency) => (0).toLocaleString(
  coalesce(nullSafeString(locale), "en-US"), {
    style: "currency",
    currency,
...
const hasProperty = (object, key) => (object ? Object.hasOwnProperty.call(
  object, key) : false);
...
const objectsDeepEqual = (o1, o2) => {
  ...
  const haveEqualLengthOfKeys = Object.keys(o1).length === Object.keys(
    o2).length;
  return areObjects && notZero && haveEqualLengthOfKeys ? stringify(o1)
    === stringify(o2) : o1 === o2;
};
...
const coalesce = (value, defaultValue) => (anyTrue(nullSafeString(value) === "",
  !isPresent(value)) ? defaultValue : value);
const trimCoalesce = (value, defaultValue) => nullSafeTrim(
...
const debounce = (func, delay = 500 * 2.5) => {
  ...
  return function () {
    ...
    debounceTimer = setTimeout((context) => func.apply(context, args), delay,
      this);
  };
};

const isColorCode = color => anyTrue(color.substring(0, 1) === "#",
  color.substring(0, 1) === "(");
...
const isValidationFailed = (validator) => anyTrue(validator.isFailed,
  validator.isRegExFailed);
...
function getFnName(fn) {
  ...
  let s = f && ((fn.name && ["",
    fn.name]) || fn.toString().match(/function ([^\(]+)/));
