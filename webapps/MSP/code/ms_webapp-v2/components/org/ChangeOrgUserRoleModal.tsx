...
interface Input {
  isOpen: boolean;
  onRequestClose: () => void;
  user: IPersonnelSearchForUI;
  onConfirm: ({ role: string, user: IPersonnelSearchForUI }) => Promise<any>;
}

const ChangeOrgUserRoleModal = ({
  isOpen,
  user,
  onConfirm,
  onRequestClose,
  ...
  return (
    ...
    >
      <ModalContent>
        <p className="smallBody">{user?.user?.name}</p>
        <Select
          id="role"
          inline={true}
          value={role}
...
export default ChangeOrgUserRoleModal;
