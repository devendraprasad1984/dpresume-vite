...
import {ClientType as ChatClientType} from "fnpm/chat/stream/enum";
import {IChatConfig} from "fnpm/chat/stream/models";
import {Meet as ExtMeet} from "fnpm/meet";
import {TwilioApi} from "fnpm/meet/twilio";
import {IMeetConfig} from "fnpm/meet/twilio/models";
import {ClientType as MeetClientType} from "fnpm/meet/twilio/enum";
...
/**
 * Notification service/function interface
 */
...
/**
 * Class derived from INotification
 */
@injectable()
export class Notification implements INotification {
    ...
    constructor(
        ...
        // setup config
        global.init(Config as unknown as IServiceConfig);
    ...
    start(): Server {
        ...
        console.info(`Notification service API Docs are running on http://localhost:${port}/notification/api-docs`);
    ...
    /**
     * Get service version from service package.json
     */
    ...
    private resolveDependencies(): void {
        // Meet service
        container.register("Meet", {useClass: ExtMeet});
        const meet: ExtMeet<TwilioApi> = new ExtMeet(
            MeetClientType.Twilio,
            this.global.config.meet as unknown as IMeetConfig
        );
        container.registerInstance("Meet", meet);

        // chat service
        container.register("Chat", {useClass: Chat});
        const chat: Chat<StreamApi> = new Chat(
            ChatClientType.Stream,
            this.global.config.chat as unknown as IChatConfig
        );
